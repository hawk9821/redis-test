package com.hawk.redis.hashConsistent;

import java.util.HashMap;
import java.util.Map;
import java.util.HashSet;

/**
 * @author zhangdonghao
 * @date 2019/5/1
 */
public class Test {
    public static void main(String[] args) {

        HashSet<String> serNode = new HashSet<String>();
        serNode.add("127.1.1.1#A");
        serNode.add("127.1.1.2#B");
        serNode.add("127.1.1.3#C");
        serNode.add("127.1.1.4#D");

        Map<String,Integer> serverNodeMap = new HashMap<String,Integer>();
        ConsistentHash<String> consistentHash =  new ConsistentHash<String>(new HashFunction(),1000,serNode);
        int count = 50000;
        for (int i = 0; i < count; i++) {
            String serverNodeName = consistentHash.getNode(i);
            // System.out.println(i + " 映射到物理节点---" + serverNodeName);
            if (serverNodeMap.containsKey(serverNodeName)){
                serverNodeMap.put(serverNodeName,serverNodeMap.get(serverNodeName) + 1);
            }else {
                serverNodeMap.put(serverNodeName,1);
            }
        }
        showServer(serverNodeMap);
        serverNodeMap.clear();
        System.out.println("-------------------- remove 127.1.1.1#A");
        consistentHash.removeNode("127.1.1.1#A");
        for (int i = 0; i < count; i++) {
            String serverNodeName = consistentHash.getNode(i);
            // System.out.println(i + " 映射到物理节点---" + serverNodeName);
            if (serverNodeMap.containsKey(serverNodeName)){
                serverNodeMap.put(serverNodeName,serverNodeMap.get(serverNodeName) + 1);
            }else {
                serverNodeMap.put(serverNodeName,1);
            }
        }

        showServer(serverNodeMap);
        serverNodeMap.clear();
        System.out.println("-------------------- add 127.1.1.5#E");
        consistentHash.addNode("127.1.1.5#E");
        for (int i = 0; i < count; i++) {
            String serverNodeName = consistentHash.getNode(i);
            // System.out.println(i + " 映射到物理节点---" + serverNodeName);
            if (serverNodeMap.containsKey(serverNodeName)){
                serverNodeMap.put(serverNodeName,serverNodeMap.get(serverNodeName) + 1);
            }else {
                serverNodeMap.put(serverNodeName,1);
            }
        }
        showServer(serverNodeMap);

    }
    public static void showServer(Map<String,Integer> map) {
        for (Map.Entry<String, Integer> m : map.entrySet()){
            System.out.println(m.getKey() + ",存储数据量 " + m.getValue());
        }
    }


}
